...
def _call_claude_api(
    ...
    if model_key not in CLAUDE_MODELS:
        logger.warning(
            f"Attempting to use non-approved model key: {model_key}. Defaulting to CONTEXT_GEN."
        )
    ...
    try:
        ...
        if (
            ...
            and "error" not in response_text.lower()[:50]
        ):
            logger.info(
                f"Claude API call successful: {task_type}, model={model}, time={elapsed_time:.2f}s"
            )
            return response_text
        else:
            logger.warning(
                f"Claude API returned unexpected response: {response_text[:100]}..."
            )
            return response_text
...
class BaseAPI:
    ...
    def _get_api_key(self) -> Optional[str]:
        ...
        try:
            ...
            logger.error(
                f"Could not import {self.config_key_name} from config.api_keys"
            )
        except Exception as e:
    ...
    def _log_api_call(self, model: str, task_type: str) -> None:
        logger.info(
            f"Calling {self.__class__.__name__}: model={model}, task_type={task_type}"
        )

    def _log_api_success(self, task_type: str, start_time: float) -> None:
        end_time = time.time()
        logger.info(
            f"{self.__class__.__name__} call successful: {task_type}, time={end_time - start_time:.2f}s"
        )
    ...
    def check_token_limits(
        ...
        model_limit = MODEL_TOKEN_LIMITS.get(
            model, 100000
        ...
        if estimated_tokens > model_limit:
            return (
                False,
                f"Content likely exceeds token limit: ~{estimated_tokens} tokens (limit: {model_limit})",
            )
        return True, ""
...
class ClaudeAPI(BaseAPI):
    ...
    def call_api(
        ...
        prompt: str,
        system_prompt: str = "",
        model: str = CLAUDE_MODELS["CONTEXT_GEN"],
        max_tokens: int = 4000,
        ...
        is_within_limits, limit_message = self.check_token_limits(
        ...
        if len(prompt) > 100000:  # Arbitrary threshold
            logger.warning(
                f"Large input detected ({len(prompt)} chars), truncating to 100K chars"
            )
        ...
        logger.info(
            f"Making Claude API request with {timeout_seconds}s timeout for model: {model}"
        )
        ...
        try:
            ...
            if response := getattr(e, "response", None):
                ...
                if response.status_code == 400:
                    logger.error(
                        f"{error_msg} - Content may be too large or violate content policy"
                    )
    ...
    def call_api(
        ...
        try:
            ...
            response_filename = os.path.join(
                response_logs_dir, f"response_{timestamp}.json"
            )
            try:
                ...
                try:
                    ...
                    if isinstance(response, dict):
                        ...
                        if isinstance(text_content, str) and text_content.startswith(
                            "Error generating content with"
                        ):
                            error_message = text_content.lower()
                            # Don't retry certain errors
                            if any(
                                err in error_message for err in non_retryable_errors
                            ):
                                logger.warning(
                                    f"Non-retryable error detected: {text_content[:100]}..."
                                )
                                return response

                            logger.warning(
                                f"API call failed on attempt {attempt}/{max_retries}: {text_content[:100]}..."
                            )
                            if attempt < max_retries:
                                logger.info(
                                    f"Retrying API call (attempt {attempt+1}/{max_retries})..."
                                )
                                # If the error is about large content, try truncating
                                if (
                                    "too large" in error_message
                                    ...
                                    if "prompt" in kwargs:
                                        ...
                                        kwargs["prompt"] = (
                                            kwargs["prompt"][:new_len]
                                            + "... [truncated]"
                                        )
                                        logger.info(
                                            f"Truncated prompt from {prompt_len} to {new_len} chars"
                                        )
                                continue
                            else:
                                logger.error(
                    ...
                    # Original string response handling
                        ...
                        if any(err in error_message for err in non_retryable_errors):
                            logger.warning(
                                f"Non-retryable error detected: {response[:100]}..."
                            )
                            return response

                        logger.warning(
                            f"API call failed on attempt {attempt}/{max_retries}: {response[:100]}..."
                        )
                        if attempt < max_retries:
                            logger.info(
                                f"Retrying API call (attempt {attempt+1}/{max_retries})..."
                            ...
                            if (
                                ...
                                if "prompt" in kwargs:
                                    ...
                                    new_len = int(prompt_len * 0.75)
                                    kwargs["prompt"] = (
                                        kwargs["prompt"][:new_len] + "... [truncated]"
                                    )
                                    logger.info(
                                        f"Truncated prompt from {prompt_len} to {new_len} chars"
                                    )
                    ...
                    last_error = str(e)
                    logger.warning(
                        f"Error on attempt {attempt}/{max_retries}: {last_error}"
                    )
                    if attempt < max_retries:
...
"""
    ...
    if model != CLAUDE_MODELS["CONTEXT_GEN"]:
        logger.warning(
            f"Model parameter override detected in generate_with_claude_haiku: {model}. Using CONTEXT_GEN model instead."
        )
...
"""
    ...
    if model != CLAUDE_MODELS["CONTENT_EDIT"]:
        logger.warning(
            f"Model parameter override detected in generate_with_claude_sonnet: {model}. Using CONTENT_EDIT model instead."
        )
        # We'll ignore the passed model parameter and use our standardized model
...
@with_retry(max_retries=3)
def generate_with_claude_extended_thinking(
    prompt: str,
    system_prompt: str = "",
    ...
    if thinking_budget < 1024:
        logger.warning(
            f"Thinking budget {thinking_budget} is less than minimum 1024. Using 1024 instead."
        )
    ...
    if temperature != 1.0:
        logger.warning(
            f"Temperature {temperature} is not 1.0. Extended Thinking requires temperature=1.0. Adjusting."
        )
        temperature = 1.0
...
@with_retry(max_retries=3)
def generate_with_claude_diff_edit(
    prompt: str,
...
"""
    ...
    try:
        ...
        if not edited_content:
            logger.warning(
                "Could not extract edited content from response, using the whole response"
            )
...
@with_retry(max_retries=3)
def edit_markdown_with_claude(
    markdown_text, instructions, context="", model=None, temperature=0.0
):
    """
    ...
    try:
        ...
        if not response.ok:
            logger.error(
                f"Error calling Claude API: {response.status_code} - {response.text}"
            )
        ...
        insert_pattern = (
            r"\[EDIT\s*:\s*INSERT\s*:\s*(\d+)\s*\]([\s\S]*?)\[/\s*EDIT\s*\]"
        )
        ...
        for match in re.finditer(replace_pattern, api_response, re.IGNORECASE):
            ...
            if 1 <= start_line <= len(lines) and 1 <= end_line <= len(
                lines
            ):  # Validate against original lines
                all_edits.append(
                    {
                        "type": "replace",
                ...
                logger.warning(
                    f"Invalid line range for replacement: {start_line}-{end_line}"
                )
        ...
        for edit in all_edits:
            if edit["type"] == "insert":
                ...
                logger.info(
                    f"Inserting content after line {line_num} ({format_type} format)"
                )
                content_lines = [
                    re.sub(r"^\[\d+\]\s*", "", ln) for ln in content.split("\n")
                ]
                ...
                logger.info(
                    f"Replacing lines {start_line}-{end_line} ({format_type} format)"
                )
        ...
        elapsed_time = time.time() - start_time
        logger.info(
            f"Direct line-based editing process completed in {elapsed_time:.2f}s"
        )
...
@with_retry(max_retries=3)
def regenerate_markdown_with_claude(
    markdown_text, instructions, context="", model=None, temperature=0.0
):
    """
