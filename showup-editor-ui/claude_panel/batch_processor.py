...
edit_markdown_with_claude = claude_api.edit_markdown_with_claude
...
class BatchProcessor:
    ...
    def setup_batch_tab(self):
        ...
        bullet1 = ttk.Label(bullet_frame, text="• The prompt from the 'Prompt Config' tab")
        bullet1.pack(anchor="w")
        
        bullet2 = ttk.Label(bullet_frame, text="• The learner profile from the 'Prompt Config' tab")
        bullet2.pack(anchor="w")
        
        bullet3 = ttk.Label(bullet_frame, text="• Files will be processed one by one with context generation")
        bullet3.pack(anchor="w")
        
        bullet4 = ttk.Label(bullet_frame, text="• Original files will be backed up with .bak extension")
        ...
        self.process_btn = ttk.Button(process_frame, text="Process Batch", command=self.process_batch_edit)
    ...
    def process_batch_edit(self):
        ...
        logger.info(f"Starting batch processing with prompt '{self.parent.prompt_manager.selected_prompt}' and profile '{selected_profile}'")
        ...
        self.batch_thread = threading.Thread(
            ...
            args=(batch_files, prompt_text, learner_profile)
        )
    ...
    def _process_files(self, files, prompt, learner_profile):
        ...
        logger.info(f"Starting batch processing of {len(files)} files with prompt '{prompt[:50]}...'")
    ...
    def _update_batch_cancelled(self, processed, total):
        ...
        self.status_var.set(f"Batch processing cancelled: {processed} of {total} files processed")
        self.process_btn.config(state="normal")
        self.cancel_btn.config(state="disabled")
        self.processing_batch = False
    ...
    def select_directory_for_batch(self):
        ...
        if files_to_add:
            ...
            messagebox.showinfo("No Files", f"No files with extensions {', '.join(extensions)} found in the selected directory.")
